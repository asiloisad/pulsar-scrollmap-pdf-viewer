...
module.exports = {
  activate() {
    ...
    this.disposables = new CompositeDisposable(
      atom.config.observe("scrollmap-pdf-viewer.maxDepth", (value) => {
        this.maxDepth = value;
      }),
      atom.config.observe("scrollmap-pdf-viewer.threshold", (value) => {
  ...
  consumePdfViewer(service) {
    if (service.hasIntegratedScrollmap) {
  ...
  updateScrollmap(ctx, data) {
    ...
    if (this.maxDepth > 0) {
      filtered = filtered.filter((item) => item.level < this.maxDepth);
    }
    if (this.threshold > 0 && filtered.length > this.threshold) {
